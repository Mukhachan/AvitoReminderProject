...
блокировки. Профиль Chromium сохраняется между запусками в `data/chromium-profile`.
...
создавала бы противоречивый отпечаток. Внешние cookies и автоматическое решение капчи не
используются. Смена IP выполняется только настроенным ниже пулом прокси и не решает капчу.
...
один раз ждёт 90 секунд и обновляет страницу. При повторной блокировке он закрывает Chromium и
HTTP-сессию, выбирает следующий прокси, ждёт 15 секунд и заново открывает Avito с тем же
постоянным профилем cookies. После пяти неудачных смен IP включается общая пауза на три часа.
Режим `proxy` начинает сразу с первого прокси, а `direct` полностью отключает пул.
...
Первый Chromium запускается через первый адрес списка; при блокировке парсер последовательно
переключается на следующие адреса по тому же алгоритму. Вернуть первоначальную схему можно
значением `AVITO_PROXY_MODE=fallback`.
