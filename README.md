...
Страница `https://www.avito.ru/#block` с заголовком «Доступ ограничен: проблема с IP» считается
жёсткой блокировкой адреса. Для неё парсер не ждёт 90 секунд и не обновляет вкладку: Chromium и
сетевые сессии сразу пересоздаются на следующем IP пула. Ожидание с reload сохраняется для HTTP
429 без признаков `#block` и страниц капчи, потому что они иногда открываются после паузы без
смены адреса.
