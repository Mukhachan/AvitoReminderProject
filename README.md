...
сохраняет тот же sticky-IP, ждёт случайные 90–120 секунд и один раз обновляет вкладку. Если Avito
открылся, поиск продолжается с этим IP. Только при повторной блокировке Chromium и сетевые сессии
пересоздаются на следующем IP пула, где применяется тот же цикл ожидания.
