...
не ждёт 90–120 секунд и не обновляет заблокированную вкладку. Он сразу сохраняет диагностику,
закрывает Chromium и сетевые сессии, создаёт новую identity и открывает Avito через следующий
IP пула. Ожидание с reload сохраняется для капчи и остальных временных ошибок.
