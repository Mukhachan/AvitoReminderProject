...
После каждого перехода на главную или поисковую страницу Chromium ждёт 90 секунд, делает
одно обязательное обновление и только после этого читает страницу. Если после обновления осталась
блокировка, он снова ждёт 90 секунд и повторяет обновление. Первый снимок ошибки сохраняется в
`data/diagnostics` и отправляется в Telegram. Если после снятия блокировки
открылась главная, парсер заново перейдёт к текущему поиску. Бот не обходит капчу автоматически;
